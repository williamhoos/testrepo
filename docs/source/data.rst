...
As indicated, the table contains a total of 18 columns, each of which has a number of properties, including the key shown to the left of the number column, which indicates that the values in that column are also indexed and the results cached to make it more efficient to query.  The gold key designates the Primary Key for the index, and each of the green keys indicates that other properties are included.  
